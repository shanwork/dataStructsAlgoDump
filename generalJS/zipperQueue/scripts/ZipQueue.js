class ZipQueue {
    ...
    addQueue(grandParentElement, parentElementType, elementType, singleElement = null, multiList = null, refresh = false ) {
        if (singleElement !== null) {
            if (this.totalQueueLength <= this.mainQMaxLength) {
                this.mainQueue.enqueue(singleElement);
            }  else  {
                if (this.totalQueueLength < this.queueCapacity) {
                    for (let subQ of this.subQueues) {
                        if (subQ.length < this.subQueueLength) {
                            subQ.enqueue(singleElement);
                            break;
                        }
                    }
                } else {
    ...
    addSingleQueueElement(singleElement) {
